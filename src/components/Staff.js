  ...
  return (
      ...
      {workers.map((elem, i) => {
      	 if(elem.id == editElementId){
      	 		return <EditInfo id = {editElementId}
      	 			worker = {elem}
      	 			posts = {posts}
  					actions = {actions}
  					key={editElementId}
  	    	...
  	    	<li className='worker' key = {elem.id}>
  	    		...
  	    		<div className='wrapper' >
  		    		<p className='initials'>{elem.first_name +' '+elem.last_name}</p>
  		    		<p className='birthDay'>{coverterTime(elem.birth_date)}</p>
  		    		<p className='rank'>{posts[elem.post - 1].name}</p>	    			    		
  		    		<button className='edit' onClick={(id = elem.id) =>{ actions.SetEditElementID(elem.id)}}>Редактировать</button>
  	    		</div>
  	    	</li>
      		)
  	    }

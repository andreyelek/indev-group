...
class EditInfo extends Component {
  ...
  handleChange(item, itemName) {
    ...
    if (!item.value.length) {
      this.setState({
        errors: { ...this.state.errors, [itemName]: "Поле не заполнено!" }
      });
    } else
      this.setState({ errors: { ...this.state.errors, [itemName]: false } });
  }
      ...
      <li className="editWorker" key={id}>
        ...
        <div className="wrapperInput">
          ...
          <label className="birthDate">
            ...
            <input
              type="date"
              defaultValue={coverterTime(worker.birth_date)}
          ...
          <label className="rank">
            <div>Звание</div>
            <select
              defaultValue={posts[worker.post - 1].id}
        ...
        <div className="buttons">
          ...
          <button
            className="delete"
            onClick={() => actions.deleteWorkerInfo(id)}
          >
            Удалить
          </button>
          <button
            className="cancell"
            onClick={() => actions.SetEditElementID(false)}
          >
            Отмена
          </button>
        </div>
